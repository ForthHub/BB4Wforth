...
: WELCOME
	." BB4Wforth version " VERSION$ TELL
...
\ Create aliases of non-standard (but useful) Jonesforth words:
...
\ Add custom BB4W words:

Z" kernel32.dll" LoadLibrary
DUP Z" CreateFileA" GetProcAddress CONSTANT CreateFile
DUP Z" CloseHandle" GetProcAddress CONSTANT CloseHandle
DUP Z" Sleep" GetProcAddress CONSTANT Sleep
FreeLibrary DROP ( kernel32 is permanently resident )

: MODE ( n -- ) 22 EMIT EMIT ;
: CLS ( -- ) 12 EMIT ;
: CLG ( -- ) 16 EMIT ;
: COLOUR ( n -- ) 17 EMIT EMIT ;
: GCOL ( c m -- ) 18 EMIT EMIT EMIT ;
: EMIT16 ( w -- ) 256 U/MOD SWAP EMIT EMIT ;
: XY ( x y -- ) SWAP EMIT16 EMIT16 ;
: ORIGIN ( x y -- ) 29 EMIT XY ;
: PLOT ( x y n -- ) 25 EMIT EMIT XY ;
: MOVETO ( x y -- ) 4 PLOT ;
: DRAW ( x y -- ) 5 PLOT ;
: FLOOD ( x y -- ) 133 PLOT ;
: MOVEBY ( x y -- ) 0 PLOT ;
: DRAWBY ( x y -- ) 1 PLOT ;
: TABXY ( x y -- ) SWAP 31 EMIT EMIT EMIT ;
: BYE ( -- ) S" BYE" OSCLI ;
: DIR ( -- ) S" DIR *.f*" OSCLI ;

: EXEC ( c-addr u -- )
  TUCK HERE 5 + SWAP CMOVE S" EXEC "
  HERE SWAP CMOVE 5 + HERE SWAP OSCLI
;

: CHAN ( hfile -- chan )
  DUP 1+ 0= IF DROP 0 EXIT THEN
  hfiles 20 + BEGIN DUP @ 0= IF TUCK ! hfiles - 4 / EXIT THEN
  4+ DUP hfiles - 48 > UNTIL DROP CloseHandle SYSCALL DROP 0
;

HEX
: OPENIN ( z-addr -- chan )
  0 80 3 0 3 80000000 6 ROLL CreateFile SYSCALL CHAN ;
: OPENOUT ( z-addr -- chan )
  0 80 1 0 2 C0000000 6 ROLL CreateFile SYSCALL CHAN ;
: OPENUP ( z-addr -- chan )
  0 80 3 0 2 C0000000 6 ROLL CreateFile SYSCALL CHAN ;
DECIMAL

: CLOSE ( chan -- )
  DUP IF
    hfiles 84 + DUP @ >R ! S" SPOOL" OSCLI R> hfiles 84 + !
  ELSE 
    13 5 DO hfiles I CELLS + @ IF I RECURSE THEN LOOP
  THEN
;
...
: EVALUATE ( i*x c-addr u -- j*x )
  ...
  SETSRC BEGIN INTERPRET SOURCE NIP >IN @ < UNTIL RESTORE
;
...
\ If a command-line argument was supplied, run that file:

: RUNCMD ( -- ) cmd$ DUP IF INCLUDED ELSE 2DROP THEN ;
RUNCMD
